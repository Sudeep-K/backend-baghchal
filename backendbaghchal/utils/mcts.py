...
class MCTS:
    ...
    def expand(self, node):
        ...
        if (node.board.player_turn == node.board.player_goat) and (
            node.board.goats["onHand"] > 0
        ):
            ...
            for row in range(5):
                for col in range(5):
                    if (node.board.player_turn == node.board.player_goat) and (
                        node.board.position[row][col] == node.board.player_goat
                    ):
                    ...
                    elif (node.board.player_turn == node.board.player_tiger) and (
                        node.board.position[row][col] == node.board.player_tiger
                    ):
                        node.board.selected_position = [row, col]
                        node.board.valid_moves = []
                        node.board.valid_strategies()
    ...
    def rollout(self, board):
        ...
        if board.is_gameover() == 1:
            return -1  # goat wins
        elif board.is_gameover() == 2:
            return 1  # tiger wins
    ...
    def get_best_move(self, node, exploration_value):
        ...
        for child_node in node.children.values():
            ...
            if child_node.board.player_turn == child_node.board.player_tiger:
                current_player = 1
            elif child_node.board.player_turn == child_node.board.player_goat:
                current_player = -1
